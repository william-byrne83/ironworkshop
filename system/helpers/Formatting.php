...
class Formatting {
	 ...
	 *
	 */
	static public function removeAccents($str, $utf8=true){
		$str = (string)$str;
		if( is_null($utf8) ) {
	...
	/**
	 *
	 * Function: utf8_htmlentities
	 * Covert string into utf8.
	 *
	 * @param $str input string
	 * @return string input string in ut8 format
	 *
	 */
	static public function utf8_htmlentities($str){
		$formatted_str = htmlspecialchars($str, ENT_QUOTES, "UTF-8");
	 ...
	 * Covert string into paragraphs.
	 *
	 * @param $str input string
	 *
	 */
	static public function addParagraphs($str){		
...
<li>", '</li><li>', $str);
	 ...
	 * Convert Youtube and Vimeo URLs
	 *
	 * @param $videoString
	 *
	 */
	static public function parseVideos($videoString = null){
		// return data
		$videos = array();
					...
					if (strpos($link, 'youtu') !== FALSE) {
						...
						if ($videoIdRegex !== NULL) {
							if (preg_match($videoIdRegex, $link, $results)) {
								$thumbnail_str = 'http://img.youtube.com/vi/%s/0.jpg';
								$fullsize_str = 'http://img.youtube.com/vi/%s/maxresdefault.jpg';
								$video_id = $results[1];
							}
						}
					}
							...
							if (preg_match($videoIdRegex, $link, $results)) {
								...
								try {
									...
									if (!empty($hash) && is_array($hash)) {
										$thumbnail_str = $hash[0]['thumbnail_medium'];
