    	...
    	<div class="block-title">
            ...
            <div class="block-options pull-right">
	    		 <div id="esearch" class="dataTables_filter">
		    		<form class="form-wrap" method='get' action='/backoffice/galleries/index'>
	        ...
	        <div class="dataTables_wrapper form-inline no-footer">
		        <div class="row">
			        <div class="col-xs-12">
                       <div class="pull-right">
                            <a href="/backoffice/galleries/add" class="btn btn-success"><i class="fa fa-plus"></i> Add Gallery</a>
                        </div>
				    </div>
				</div>
                                    ...
                                    <tr <?php if($data['is_active'] == 0){ echo 'class="danger"'; } ?>>
                                        <td>
                                            <?php if(count($this->getAllData) > 1){?>
                                                ...
                                                <?php if($a != 1){?>
                                                    <a class="btn btn-sm btn-info" href="/backoffice/galleries/sort/up/<?php echo $data['id']; ?>/<?php echo $data['sort']?>" ><i class="fa fa-caret-up"></i></a>
                                                <?php } ?>

                                                <!--if its not the last item add down arrow-->
                                                <?php if ($a != count($this->getAllData)){?>
                                                    <a class="btn btn-sm btn-info" href="/backoffice/galleries/sort/down/<?php echo $data['id']; ?>/<?php echo $data['sort']?>" ><i class="fa fa-caret-down"></i></a>
                                                <?php } ?>
                                            <?php } ?>
                                        </td>
                                        ...
                                        <td>
                                            <?php if(!empty($data['image'])){?>
                                                <img src="/assets/uploads/galleries/<?php echo $data['image']?>" alt="<?php echo $data['title']?>" style = "width:64px;">
                                            <?php } ?>
                                        </td>

                                        <td class="text-left">
                                            <a href="/backoffice/galleries/edit/<?php echo $data['id']; ?>/" data-toggle="tooltip" title="Edit Gallery" class="btn btn-effect-ripple btn-sm btn-success"><i class="fa fa-pencil"></i></a>
                                            <a href="/backoffice/galleries/delete/<?php echo $data['id']; ?>/" data-toggle="tooltip" title="Delete Gallery" class="btn btn-effect-ripple btn-sm btn-danger"><i class="fa fa-times"></i></a>
                                        </td>
                                    </tr>
                                    <?php $a++;?>
                                <?php } ?>
	            ...
	            <?php }else{ ?>
		             <div class="row no-result">
			            <div class="col-xs-12">
							<p>There are no galleries to display.</p>
			            </div>
		            </div>
			    <?php } ?>
